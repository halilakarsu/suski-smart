...
class KoyMerkezExport implements FromView, WithColumnWidths, WithEvents, WithTitle
{
    ...
    protected function styleTitle($sheet): void
    {
        ...
        $sheet->setCellValue('A1', 'Köy ve Merkeze Yönelik Özet Bilgiler');
    }
    ...
    protected function styleFilters($sheet): void
    {
        ...
        $bolge = empty($this->filters['bolge']) ? 'Tümü' : (is_array($this->filters['bolge']) ? implode(', ', $this->filters['bolge']) : $this->filters['bolge']);
        ...
        $tarife = empty($this->filters['tarife']) ? 'Tümü' : (is_array($this->filters['tarife']) ? implode(', ', $this->filters['tarife']) : $this->filters['tarife']);

        $sheet->setCellValue('A2', "Bölge: {$bolge}  |  Dönem: {$donem}  |  Tarife: {$tarife}");
    }
    ...
    protected function styleHeaderGroupRow($sheet): void
    {
        ...
        $sheet->getStyle('D3:E3')->getFill()
            ->setFillType(Fill::FILL_SOLID)
            ->getStartColor()->setRGB('2563EB');

        $sheet->getStyle('F3:G3')->getFill()
            ->setFillType(Fill::FILL_SOLID)
    ...
    protected function styleHeaderSubRow($sheet): void
    {
        ...
        $sheet->getStyle('D4:E4')->getFill()
            ->setFillType(Fill::FILL_SOLID)
            ->getStartColor()->setRGB('2563EB');

        $sheet->getStyle('F4:G4')->getFill()
            ->setFillType(Fill::FILL_SOLID)
            ->getStartColor()->setRGB('16A34A');

        $sheet->getStyle('H4:I4')->getFill()
            ->setFillType(Fill::FILL_SOLID)
            ->getStartColor()->setRGB('D97706');
    }
    ...
    protected function styleDataRows($sheet, int $dataEnd): void
    {
        ...
        for ($row = self::DATA_ROW_START; $row <= $dataEnd; $row++) {
            ...
            $sheet->getStyle("E{$row}")->getFont()->getColor()->setRGB('2563EB');
