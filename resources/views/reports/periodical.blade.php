...
<style>
    ...
    .periodical-spinner {
        ...
        animation:periodicalSpin .85s linear infinite; margin:0 auto 20px;
    }
    @keyframes periodicalSpin { to{transform:rotate(360deg);} }
    .periodical-loader-title { font-size:1.15rem; font-weight:800; color:#0f172a; margin-bottom:6px; }
    .periodical-loader-sub   { font-size:.85rem; color:#64748b; font-weight:500; }
    .periodical-progress     { width:220px; height:6px; background:#e2e8f0; border-radius:99px; margin:16px auto 0; overflow:hidden; }
    .periodical-progress-bar {
        height:100%; width:0%;
        background:linear-gradient(90deg,#2563eb,#4f46e5); border-radius:99px;
        animation:periodicalProgressFill 30s ease-out forwards;
    }
    @keyframes periodicalProgressFill { 0%{width:0%;} 40%{width:55%;} 70%{width:78%;} 90%{width:90%;} 100%{width:95%;} }
    .periodical-overlay-close {
    ...
    .periodical-overlay-close:hover { background:#f1f5f9; color:#0f172a; }
    .periodical-open-btn {
    ...
    .periodical-open-btn:hover { transform:translateY(-2px); box-shadow:0 12px 24px -5px rgba(37,99,235,.45); }
    .periodical-success-icon {
...
<div class="pg-premium p-0">
    <div class="page-hero" style="padding-bottom: 7rem;">
        <div class="hero-container" style="flex-wrap: wrap; gap: 20px;">
            ...
            <div id="periodicalFilterForm" data-action="{{ route('reports.periodical') }}" class="premium-actions-grid">
                <button type="button" class="btn-premium-action btn-premium-filter" data-toggle="modal" data-target="#perAdvModal">
                    <i class="fas fa-sliders-h"></i> FİLTRELE
                    @if($hasAdvFilter)<span class="adv-active-dot"></span>@endif
                </button>

                <button type="button" id="periodical-export-pdf" class="btn-premium-action btn-premium-pdf">
                    <i class="fas fa-file-pdf"></i> PDF İndir
                </button>

                <button type="button" id="periodical-export-excel" class="btn-premium-action btn-premium-excel">
    ...
    <div class="main-container" style="margin-top: -3.5rem;">
        <div id="reportResultsContainer">
            @if($results->count() > 0)
                @include('reports.partials.periodical_table', ['results' => $results, 'totals' => $totals, 'donemler' => $donemler])
            @else
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width: 65%;" role="document">
        <div class="modal-content" style="border-radius:28px; border:1px solid rgba(255,255,255,0.2); overflow:hidden; box-shadow:0 40px 100px rgba(0,0,0,0.25); background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(20px);">
                ...
                <div class="row">
                    <div class="col-md-6" style="margin-bottom: 25px;">
                        ...
                        <div class="dropdown custom-multi-select modal-ms">
                            <button class="dropdown-toggle" type="button" id="ModalPerBolgeDropdown" data-toggle="dropdown" style="padding: 12px 18px; font-size: 0.95rem; border-radius: 12px;">
                                <span id="ModalPerBolgeLabel">Bölge Seçin...</span>
                            ...
                            <div class="dropdown-menu" onclick="event.stopPropagation();" style="border-radius: 16px; padding: 12px; border: 1px solid #e2e8f0; box-shadow: 0 20px 40px rgba(0,0,0,0.08);">
                                <div class="form-check select-all-wrap" id="selectAllModalPerBolgeRow" style="padding-bottom: 10px; border-bottom: 2px solid #f1f5f9; margin-bottom: 10px;">
                                    <input class="form-check-input" type="checkbox" id="selectAllModalPerBolge">
                                    <span class="cb-box"><svg viewBox="0 0 12 10"><polyline points="1,5 4,9 11,1"/></svg></span>
                                    <label class="form-check-label fw-bold" style="color: #0f172a;" for="selectAllModalPerBolge">Tümünü Seç</label>
                                </div>
                                @foreach($bolgeler as $bolge)
                                    <div class="form-check modal-per-bolge-row" onclick="toggleCheckbox(this)">
                                        <input class="form-check-input modal-per-bolge-cb" type="checkbox" value="{{ $bolge }}" id="modalperbolge_{{ $loop->index }}"
                                            {{ (!request()->has('bolge') || (is_array(request('bolge')) && in_array($bolge, request('bolge')))) ? 'checked' : '' }}>
                                        <span class="cb-box"><svg viewBox="0 0 12 10"><polyline points="1,5 4,9 11,1"/></svg></span>
                                        <label class="form-check-label" for="modalperbolge_{{ $loop->index }}">{{ $bolge }}</label>
                                    </div>
                                @endforeach
                            </div>
                        </div>
            ...
            <div class="modal-footer" style="background: rgba(248, 250, 252, 0.8); border-top: 1px solid rgba(226, 232, 240, 0.8); padding: 25px 35px; display:flex; justify-content:space-between; align-items: center; border-bottom-left-radius: 28px; border-bottom-right-radius: 28px;">
                <button type="button" class="btn-pro btn-outline-pro" id="perClearBtn" style="border-radius: 12px; font-weight: 700; transition: all 0.2s;"><i class="fas fa-eraser"></i> Filtreleri Temizle</button>
                <div class="d-flex gap-3">
                    <button type="button" class="btn-pro btn-primary-pro" id="perApplyBtn" style="border-radius: 12px; font-weight: 700; padding-left: 28px; padding-right: 28px; transition: all 0.2s;"><i class="fas fa-check"></i> Sonuçları Getir</button>
                </div>
            </div>
        </div>
    </div>
...
<div id="periodicalExportOverlay">
    <div class="periodical-loader-box">
        <div id="periodicalSuccessIcon" class="periodical-success-icon"><i class="fas fa-check"></i></div>
        <div id="periodicalSpinner" class="periodical-spinner"></div>
        <h4 id="periodicalLoaderTitle" class="periodical-loader-title">Rapor Hazırlanıyor</h4>
        <p id="periodicalLoaderSub" class="periodical-loader-sub">Veriler derleniyor, lütfen bekleyin...</p>

        <div id="periodicalProgressWrap" class="periodical-progress">
            <div id="periodicalProgressBar" class="periodical-progress-bar"></div>
        </div>

        <button type="button" id="periodicalOpenBtn" class="periodical-open-btn"><i class="fas fa-external-link-alt"></i> Dosyayı Aç</button>
        <button type="button" id="periodicalOverlayClose" class="periodical-overlay-close">Kapat</button>
    </div>
</div>
...
function showOverlay(type) { 
    const $bar = $('#periodicalProgressBar');
    $bar.removeClass('periodical-progress-bar'); void $bar[0].offsetWidth; $bar.addClass('periodical-progress-bar');
    $('#periodicalSuccessIcon').hide();
    $('#periodicalSpinner').show();
    $('#periodicalProgressWrap').show();
    $('#periodicalOpenBtn').hide();
    $('#periodicalOverlayClose').hide();
    $('#periodicalExportOverlay').addClass('active');
    setTimeout(() => { $('#periodicalOverlayClose').show(); }, 10000);
}
function hideOverlay() { 
    $('#periodicalExportOverlay').removeClass('active'); 
...
function buildFormData() {
    ...
    // Bölge
    if (!$('#selectAllModalPerBolge').is(':checked')) {
        const modalBolge = $('.modal-per-bolge-cb:checked').map(function(){ return $(this).val(); }).get();
    ...
    // Tarife
    if (!$('#selectAllPerTarife').is(':checked')) {
        $('.per-tarife-cb:checked').each(function(){ params.append('tarife[]', $(this).val()); });
    }

    // Bağlantı Grubu
    const baglantiGrp = $('#per_baglanti').val();
    if (baglantiGrp) params.set('baglanti_grubu', baglantiGrp);

    // Yerleşim Türü
    const yerlesim = $('#per_yerlesim').val();
    if (yerlesim) params.set('yerlesim_tipi', yerlesim);

    // Tesisat No
    const tesisat = $('#per_tesisat').val().trim();
...
async function handleExport(type) {
    lastExportType = type;
    const form = document.getElementById('periodicalFilterForm');
    const params = buildFormData();
    params.append('export', type);
    const actionUrl = form ? form.getAttribute('data-action') : "{{ route('reports.periodical') }}";
    ...
    try {
        ...
        $('#periodicalSpinner').hide();
        $('#periodicalProgressWrap').hide();
        $('#periodicalSuccessIcon').css('display','flex');
        $('#periodicalLoaderTitle').text('İndirme Hazır!');
        $('#periodicalLoaderSub').text('Excel dosyanız başarıyla oluşturuldu.');
        $('#periodicalOpenBtn').html('<i class="fas fa-file-excel"></i> Excel Dosyasını İndir').show();
        $('#periodicalOverlayClose').show();
    } catch (e) {
        $('#periodicalLoaderTitle').text('Hata Oluştu');
        $('#periodicalLoaderSub').text('Rapor oluşturulurken bir hata meydana geldi.');
        $('#periodicalSpinner').hide();
        $('#periodicalProgressWrap').hide();
        $('#periodicalOverlayClose').show();
    }
...
$(document).ready(function() {
    ...
    initMS('selectAllModalPerBolge','modal-per-bolge-cb','ModalPerBolgeLabel','Bölge Seçin...','Tüm Bölgeler Seçili','Bölge Seçili');
    ...
    $('#periodical-export-pdf').click(() => handleExport('pdf'));
    $('#periodical-export-excel').click(() => handleExport('excel'));
    $('#periodicalOpenBtn').click(() => { 
    ...
    $('#periodicalOverlayClose').click(hideOverlay);

    $('#perClearBtn').click(function() {
        $('.modal-per-bolge-cb').prop('checked', false).trigger('change');
    ...
    $('#perApplyBtn').on('click', function() {
        var modalHasBolge    = $('.modal-per-bolge-cb:checked').length > 0;
        ...
        const params = buildFormData();
        const actionUrl = $('#periodicalFilterForm').attr('data-action') || "{{ route('reports.periodical') }}";
