    ...
    <div class="page-hero">
        <div class="hero-container">
            <div class="hero-title-group">
                <h1 class="hero-title">Ekipler</h1>
                <p class="hero-subtitle">Sistemdeki ekipleri görüntüleyin ve yönetin.</p>
            </div>
        </div>
    </div>
        ...
        <div class="glass-card">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h5 class="card-title-pro mb-0">
                    ...
                    Ekip Listesi
                    <span class="tab-count-pro" style="margin-left:8px;">{{ $ekips->total() }}</span>
                </h5>
                <a href="{{ route('tesis-bilgi-sistemi.ekip.create') }}" class="btn-pro btn-primary-pro">
                    <i class="fas fa-plus"></i> Yeni Ekip
                </a>
            </div>
                ...
                <table class="table-pro">
                    ...
                    <tbody>
                        ...
                        <tr id="ekip-row-{{ $e->id }}">
                            ...
                            <td><span class="badge-pro badge-purple">{{ $e->ad }}</span></td>
                            <td style="text-align: right;">
                                <div class="d-flex justify-content-end" style="gap:6px;">
                                    <a href="{{ route('tesis-bilgi-sistemi.ekip.edit', $e->id) }}" class="action-btn-text ab-purple" style="font-size:0.75rem; padding:0 12px; text-decoration:none;">
                                        <i class="fas fa-pen"></i>
                                    </a>
                                    <button type="button" class="action-btn-text ab-gray" style="font-size:0.75rem; padding:0 12px; color:#dc2626;" onclick="confirmDelete({{ $e->id }})">
                        ...
                        <tr>
                            <td colspan="3" class="text-center" style="padding: 60px;">
                                ...
                                <p style="color: #94a3b8; font-weight: 600;">Henüz ekip eklenmemiş.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <div style="margin-top: 25px; padding-top: 20px; border-top: 1px solid #f1f5f9;">
                {{ $ekips->links('pagination::bootstrap-4') }}
            </div>
            @endif
        </div>
    </div>
...
<script>
    ...
    function confirmDelete(id) {
        Swal.fire({
            title: 'Emin misiniz?',
            text: "Bu ekibi silmek istediğinizden emin misiniz?",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '/tesis-bilgi-sistemi/ekip/' + id,
                    type: 'POST',
                    data: { _method: 'DELETE' },
                    success: function() {
                        var row = $('#ekip-row-' + id);
