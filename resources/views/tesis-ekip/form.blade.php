...
<div class="pg-premium p-0">
    <div class="page-hero">
        <div class="hero-container">
            <div class="hero-title-group">
                <h1 class="hero-title">{{ isset($ekip) ? 'Ekip Düzenle' : 'Yeni Ekip' }}</h1>
                <p class="hero-subtitle">{{ isset($ekip) ? 'Mevcut ekibi güncelleyin.' : 'Sisteme yeni bir ekip ekleyin.' }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="main-container">
        <div class="glass-card">
            <form action="{{ isset($ekip) ? route('tesis-bilgi-sistemi.ekip.update', $ekip->id) : route('tesis-bilgi-sistemi.ekip.store') }}" method="POST">
                @csrf
                @if(isset($ekip)) @method('PUT') @endif

                <div class="row">
                    <div class="col-md-6 mb-4">
                        <label class="form-label">Ekip Adı</label>
                        <input type="text" name="ad" class="form-control-pro" value="{{ old('ad', $ekip->ad ?? '') }}" required placeholder="Örn: Ekip 1">
                    </div>
                </div>
                ...
                <div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #e2e8f0; display: flex; gap: 12px; justify-content: flex-end;">
                    <a href="{{ route('tesis-bilgi-sistemi.ekip') }}" class="btn-pro btn-outline-pro">İptal</a>
