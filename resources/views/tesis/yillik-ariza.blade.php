...
<div class="pg-premium p-0">
    <div class="page-hero" style="padding-bottom: 7rem;">
        <div class="hero-container" style="flex-wrap: wrap; gap: 20px;">
            <div class="hero-title-group" style="flex-grow: 1;">
                <h1 class="hero-title">İlçelere Göre Yıllık Arıza Raporu</h1>
                <p class="hero-subtitle">Yıl, bölge, arıza türü ve ekibe göre arıza kayıtlarını raporlayın.</p>

                @php
                    $hasAnyFilter = request()->anyFilled(['bolge','yil','ariza_turu','ekip']);
                    $activeBadges = [];
                    if(request('bolge')) $activeBadges[] = 'Bölge: '.count(request('bolge')).' seçili';
                    if(request('yil')) $activeBadges[] = 'Yıl: '.request('yil');
                    if(request('ariza_turu')) $activeBadges[] = 'Arıza Türü: '.count(request('ariza_turu')).' seçili';
                    if(request('ekip')) $activeBadges[] = 'Ekip: '.count(request('ekip')).' seçili';
                @endphp
                @if($hasAnyFilter)
            ...
            <div id="yearlyFilterForm" data-action="{{ route('tesis-bilgi-sistemi.ariza-raporlari.yillik-ariza') }}" class="premium-actions-grid">
                <button type="button" class="btn-premium-action btn-premium-filter" data-toggle="modal" data-target="#arizaAdvModal">
    ...
    <div class="main-container" style="margin-top: -3.5rem;">
        <div id="reportResultsContainer">
            @if($results->count() > 0)
                @include('tesis.yillik-ariza-table', ['results' => $results])
            @else
                <div class="glass-card" style="text-align:center;padding:60px 40px;">
                    <div style="width:80px;height:80px;background:#fef2f2;color:#dc2626;border-radius:24px;display:flex;align-items:center;justify-content:center;font-size:2.5rem;margin:0 auto 20px;">⚠️</div>
                    <h4 style="font-weight:800;color:var(--text-slate-900);">İlçelere Göre Yıllık Arıza Raporu</h4>
...
{{-- FİLTRE MODALI --}}
<div class="modal fade" id="arizaAdvModal" tabindex="-1" role="dialog" aria-hidden="true" style="backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); background: rgba(15, 23, 42, 0.4); overflow: visible !important;">
        ...
        <div class="modal-content" style="border-radius:28px; border:1px solid rgba(255,255,255,0.2); overflow:visible; box-shadow:0 40px 100px rgba(0,0,0,0.25); background: rgba(255, 255, 255, 0.95); backdrop-filter: blur(20px);">
            ...
            <div class="modal-body" style="padding:40px 35px;">
                <div class="row">
                    ...
                    <div class="col-md-6" style="margin-bottom: 10px;">
                        <label style="display:block; font-size:0.82rem; font-weight:800; color:#475569; margin-bottom:10px; text-transform:uppercase; letter-spacing:0.04em;">
                            <i class="far fa-calendar-alt" style="color:#64748b; margin-right:6px; font-size:1rem; vertical-align:middle;"></i> Yıl
                        </label>
                        <select id="modal_yil" class="form-control-pro" style="padding: 12px 18px; border-radius: 12px; height: 47px;">
                ...
                <div class="row">
                    <div class="col-md-6" style="margin-bottom: 10px;">
                        <label style="display:block; font-size:0.82rem; font-weight:800; color:#475569; margin-bottom:10px; text-transform:uppercase; letter-spacing:0.04em;">
                            <i class="fas fa-tools" style="color:#dc2626; margin-right:6px; font-size:1rem; vertical-align:middle;"></i> Arıza Türü
                        </label>
                        <div class="dropdown custom-multi-select modal-ms">
                            <button class="dropdown-toggle" type="button" id="ModalArizaTuruDropdown" data-toggle="dropdown" style="padding: 12px 18px; font-size: 0.95rem; border-radius: 12px; height: 47px;">
                                <span id="ModalArizaTuruLabel">Tümü</span>
                                <i class="fas fa-chevron-down" style="font-size:0.8rem; color:#94a3b8;"></i>
                            </button>
                            <div class="dropdown-menu" onclick="event.stopPropagation();" style="border-radius: 16px; padding: 12px; border: 1px solid #e2e8f0; box-shadow: 0 20px 40px rgba(0,0,0,0.08);">
                                <div class="form-check select-all-wrap" id="selectAllModalArizaTuruRow" style="padding-bottom: 10px; border-bottom: 2px solid #f1f5f9; margin-bottom: 10px;">
                                    <input class="form-check-input" type="checkbox" id="selectAllModalArizaTuru">
                                    <span class="cb-box"><svg viewBox="0 0 12 10"><polyline points="1,5 4,9 11,1"/></svg></span>
                                    <label class="form-check-label fw-bold" style="color: #0f172a;" for="selectAllModalArizaTuru">Tümünü Seç</label>
                                </div>
                                @foreach($arizaTurleri as $at)
                                    <div class="form-check modal-ariza-turu-row" onclick="toggleCheckbox(this)">
                                        <input class="form-check-input modal-ariza-turu-cb" type="checkbox" value="{{ $at }}" id="modalarizaturu_{{ $loop->index }}"
                                            {{ (is_array(request('ariza_turu')) && in_array($at, request('ariza_turu'))) ? 'checked' : '' }}>
                                        <span class="cb-box"><svg viewBox="0 0 12 10"><polyline points="1,5 4,9 11,1"/></svg></span>
                                        <label class="form-check-label" for="modalarizaturu_{{ $loop->index }}">{{ $at }}</label>
                                    </div>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="col-md-6" style="margin-bottom: 10px;">
                        <label style="display:block; font-size:0.82rem; font-weight:800; color:#475569; margin-bottom:10px; text-transform:uppercase; letter-spacing:0.04em;">
                            <i class="fas fa-users" style="color:#059669; margin-right:6px; font-size:1rem; vertical-align:middle;"></i> Ekip
                        </label>
                        <div class="dropdown custom-multi-select modal-ms">
                            <button class="dropdown-toggle" type="button" id="ModalEkipDropdown" data-toggle="dropdown" style="padding: 12px 18px; font-size: 0.95rem; border-radius: 12px; height: 47px;">
                                <span id="ModalEkipLabel">Tümü</span>
                                <i class="fas fa-chevron-down" style="font-size:0.8rem; color:#94a3b8;"></i>
                            </button>
                            <div class="dropdown-menu" onclick="event.stopPropagation();" style="border-radius: 16px; padding: 12px; border: 1px solid #e2e8f0; box-shadow: 0 20px 40px rgba(0,0,0,0.08);">
                                <div class="form-check select-all-wrap" id="selectAllModalEkipRow" style="padding-bottom: 10px; border-bottom: 2px solid #f1f5f9; margin-bottom: 10px;">
                                    <input class="form-check-input" type="checkbox" id="selectAllModalEkip">
                                    <span class="cb-box"><svg viewBox="0 0 12 10"><polyline points="1,5 4,9 11,1"/></svg></span>
                                    <label class="form-check-label fw-bold" style="color: #0f172a;" for="selectAllModalEkip">Tümünü Seç</label>
                                </div>
                                @foreach($ekipler as $ekip)
                                    <div class="form-check modal-ekip-row" onclick="toggleCheckbox(this)">
                                        <input class="form-check-input modal-ekip-cb" type="checkbox" value="{{ $ekip }}" id="modalekip_{{ $loop->index }}"
                                            {{ (is_array(request('ekip')) && in_array($ekip, request('ekip'))) ? 'checked' : '' }}>
                                        <span class="cb-box"><svg viewBox="0 0 12 10"><polyline points="1,5 4,9 11,1"/></svg></span>
                                        <label class="form-check-label" for="modalekip_{{ $loop->index }}">{{ $ekip }}</label>
                                    </div>
                                @endforeach
                            </div>
                        </div>
            ...
            <div class="modal-footer" style="background: rgba(248, 250, 252, 0.8); border-top: 1px solid rgba(226, 232, 240, 0.8); padding: 25px 35px; display:flex; justify-content:space-between; align-items: center; border-bottom-left-radius: 28px; border-bottom-right-radius: 28px;">
                <button type="button" class="btn-pro btn-outline-pro" id="arizaClearBtn" style="border-radius: 12px; font-weight: 700; transition: all 0.2s;"><i class="fas fa-eraser"></i> Filtreleri Temizle</button>
                <div class="d-flex gap-3">
                    <button type="button" class="btn-pro btn-primary-pro" id="arizaApplyBtn" style="border-radius: 12px; font-weight: 700; padding-left: 28px; padding-right: 28px; transition: all 0.2s;"><i class="fas fa-check"></i> Sonuçları Getir</button>
                </div>
            </div>
        </div>
    </div>
...
function buildFormData() {
    ...
    if (!$('#selectAllModalBolge').is(':checked')) {
        $('.modal-bolge-cb:checked').each(function(){ params.append('bolge[]', $(this).val()); });
    }

    const yil = $('#modal_yil').val();
    if (yil) params.set('yil', yil);

    if (!$('#selectAllModalArizaTuru').is(':checked')) {
        $('.modal-ariza-turu-cb:checked').each(function(){ params.append('ariza_turu[]', $(this).val()); });
    }

    if (!$('#selectAllModalEkip').is(':checked')) {
        $('.modal-ekip-cb:checked').each(function(){ params.append('ekip[]', $(this).val()); });
    }
...
async function handleExport(type) {
    ...
    if (type === 'pdf') {
        ...
        const actionUrl = document.getElementById('yearlyFilterForm').getAttribute('data-action');
...
$(document).ready(function() {
    ...
    initMS('selectAllModalArizaTuru','modal-ariza-turu-cb','ModalArizaTuruLabel','Tümü','Tüm Arıza Türleri Seçili','Arıza Türü Seçili');
    initMS('selectAllModalEkip','modal-ekip-cb','ModalEkipLabel','Tümü','Tüm Ekipler Seçili','Ekip Seçili');
    ...
    $('#yearlyOpenBtn').click(function() {
        if (lastExportType === 'pdf') {
            ...
            a.download = 'Yillik_Ariza_Raporu.xlsx';
    ...
    $('#arizaClearBtn').click(function() {
        $('.modal-bolge-cb').prop('checked', false).trigger('change');
        $('.modal-ariza-turu-cb').prop('checked', false).trigger('change');
        $('.modal-ekip-cb').prop('checked', false).trigger('change');
        $('#modal_yil').val('');
    });

    $('#arizaApplyBtn').click(function() {
        var hasBolge = $('.modal-bolge-cb:checked').length > 0;
        var hasYil = !!$('#modal_yil').val();
        var hasArizaTuru = $('.modal-ariza-turu-cb:checked').length > 0;
        var hasEkip = $('.modal-ekip-cb:checked').length > 0;
        if (!hasBolge && !hasYil && !hasArizaTuru && !hasEkip) {
            Swal.fire({icon: 'warning', title: 'Uyarı', text: 'Lütfen en az bir filtreleme seçiniz.', confirmButtonText: 'Tamam'});
            return;
        }

        const params = buildFormData();
        const actionUrl = $('#yearlyFilterForm').attr('data-action') || "{{ route('tesis-bilgi-sistemi.ariza-raporlari.yillik-ariza') }}";
